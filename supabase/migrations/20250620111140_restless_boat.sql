/*
  ...
  2. Security
    ...
    - Create storage bucket with policies for employee files
...
-- Schedules table (using 'date' column name to match existing schema)
...
DO $$
BEGIN
  -- Add foreign key for employee_documents if not exists
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'employee_documents_employee_id_fkey'
  ...
  -- Add foreign key for time_logs if not exists
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
  ...
  -- Add foreign key for payrolls if not exists
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'schedules_employee_date_time_unique'
    AND table_name = 'schedules'
  ) THEN
    ALTER TABLE schedules 
    ADD CONSTRAINT schedules_employee_date_time_unique 
    UNIQUE(employee_id, date, shift_start_time);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'payrolls_employee_period_unique'
    AND table_name = 'payrolls'
  ) THEN
    ALTER TABLE payrolls 
