/*
  ...
  1. Table Structure
    - Create app_settings table with proper JSONB support
    - Add unique constraint on key column safely
    - Add performance indexes

  2. Data Initialization
    - Clean up any malformed configuration data
    - Initialize default application settings
    - Set up company information defaults

  3. Security
    - Enable RLS on app_settings table
    - Create policies for admin management and user viewing
*/
...
-- Add unique constraint only if it doesn't exist (using a simpler approach)
DO $$
BEGIN
...
-- Clean up any existing malformed data (fixed JSONB comparison)
