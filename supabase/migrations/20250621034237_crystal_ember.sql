/*
  # Fix Profile Role Type Mismatch

  This migration resolves the "COALESCE types text and app_role cannot be matched" error by:

  1. Database Schema Changes
     - Update the `profiles` table `role` column from `text` to `app_role` enum type
     - Set proper default value for the role column
     - Handle existing data by casting to the new type

  2. Function Updates
     - Recreate the `get_user_role` function to work with the corrected type
     - Ensure proper type handling and fallback values

  3. RLS Policy Recreation
     - Drop and recreate all RLS policies that depend on the role column
     - Ensure policies work correctly with the new `app_role` enum type

  4. Data Migration
     - Safely convert existing role data to the new enum type
     - Handle any invalid role values by setting them to 'user'
*/
